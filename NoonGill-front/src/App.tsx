...
function RoutePage() {
  ...
  return <div className="app-shell">
    <header className="topbar"><a className="brand" href="/"><span className="brand-mark">눈</span><span><strong>눈길</strong><small>숙명여대 지름길</small></span></a>
      <nav><a className="active" href="#route">길찾기</a><a href="#places">장소</a><a href="/admin/map-editor">지도 편집</a></nav><button className="profile-button">숙</button></header>
    <main>
      <section className="route-panel" id="route"><div className="route-panel-heading"><div><span className="eyebrow">CAMPUS ROUTE</span><h1>어디로 갈까요?</h1><p>직접 구축한 교내 Node·Edge 데이터로 찾는 길</p></div><span className="weather">☂ 비 오는 날 · 실내 길 추천</span></div>
      ...
      <section className="workspace"><div className="map-card"><div className="map-toolbar"><b>숙명여대 캠퍼스</b><span>{route ? `그래프 노드 ${route.points.length}개` : '경로 대기 중'}</span></div>
        ...
        <aside className="result-card"><div className="result-header"><div><span className="result-badge">{modes.find(v=>v.id===mode)?.label}</span><h2>{startPlace.name} <span>→</span> {endPlace.name}</h2></div><button onClick={()=>setSaved(!saved)} className={saved?'saved':''}>{saved?'★':'☆'}</button></div>
          ...
          {route && <><div className="route-summary"><strong>약 {Math.max(1,Math.ceil(route.estimatedSeconds/60))}분</strong><span>{Math.round(route.totalDistanceMeters)}m</span><span>실내 {Math.round(route.indoorRatio*100)}%</span></div>
            <div className="route-tags"><span>DB 저장 경로</span><span>Dijkstra 계산 결과</span></div>
          ...
          <button className="start-guide">경로 안내 시작 <span>→</span></button></aside></section>
